...
export function guardarReserva(reserva: ReservaCompleta): boolean {
  try {
    ...
    // Disparar evento para que otros componentes (como el Calendario) se enteren del cambio instantáneamente
        ...
        import("./firebase-adapter").then(({ guardarReservaFirebase }) => {
          guardarReservaFirebase(nuevaReserva)
            .then(() => {
              ...
              const idx = reservasActuales.findIndex(r => r.id === nuevaReserva.id)
              if (idx >= 0) {
                reservasActuales[idx].sincronizado = true
                localStorage.setItem(RESERVAS_STORAGE_KEY, JSON.stringify(reservasActuales))
              }
