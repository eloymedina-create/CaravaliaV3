...
export default function ConfirmacionReservaPage({ params }: { params: Promise<{ modelo: string }> }) {
  ...
  useEffect(() => {
    ...
    if (parsedDetalles && parsedCliente) {
      try {
        const fechaEntrega = parsedDetalles.fechaEntrega ? new Date(parsedDetalles.fechaEntrega) : new Date()
        const fechaDevolucion = parsedDetalles.fechaDevolucion ? new Date(parsedDetalles.fechaDevolucion) : new Date()
        
        // Evitar Invalid Date
        const isValidDate = (d: Date) => d instanceof Date && !isNaN(d.getTime())
        const fEnt = isValidDate(fechaEntrega) ? fechaEntrega : new Date()
        const fDev = isValidDate(fechaDevolucion) ? fechaDevolucion : new Date()
        
        const startDate = new Date(fEnt.getFullYear(), fEnt.getMonth(), fEnt.getDate())
        const endDate = new Date(fDev.getFullYear(), fDev.getMonth(), fDev.getDate())
        ...
        // USAR VALORES CALCULADOS DE DETALLES SI EXISTEN
        const importeTotal = parsedDetalles.importeTotal !== undefined ? parsedDetalles.importeTotal : ((precioDiario * dias) + suplemento)
        
        const config = obtenerConfiguracion()
        const senalTeorica = Math.round(((precioDiario * dias) * (config.porcentajeSenal / 100)) / 10) * 10
        const importeSenal = parsedDetalles.importeSenal !== undefined ? parsedDetalles.importeSenal : Math.max(0, senalTeorica - importeVale)
        ...
        console.error("Error al preparar pdfData:", err)
        // Fallback seguro para evitar spinner infinito
        setPdfData({
          numeroReserva,
          modelo,
          reservaDetalles: parsedDetalles,
          clienteData: parsedCliente,
          totalDias: 1,
          importeTotal: 0,
          importeSenal: 0,
    ...
    try {
      ...
      const reservaCompleta: ReservaCompleta = {
        ...
        detalles: {
          ...reservaDetalles,
          formaPago,
          porcentaje: pdfData.importeTotal > 0 ? Math.round((pdfData.importeSenal / pdfData.importeTotal) * 100) : 0,
        },
        cliente: clienteData,
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="mt-2 bg-[#F5F5F5] rounded-[1.5rem] p-5 flex flex-col gap-3 relative">
              <div className="flex justify-between items-center">
                 <span className="text-[11px] font-headline tracking-widest text-[#707974] font-bold uppercase">PRECIO CALCULADO</span>
              ...
              <div className="w-full border-t border-dashed border-[#D1D5D2]"></div>
                  <div className="flex justify-between items-center">
                     <span className="text-[11px] font-headline tracking-widest text-[#003829] font-bold uppercase">
                       {pdfData.importeSenal === 0 ? 'SEÑAL CUBIERTA' : 'SEÑAL A PAGAR'}
                     </span>
