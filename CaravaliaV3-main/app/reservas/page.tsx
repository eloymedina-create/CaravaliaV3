...
export default function ReservasPage() {
  ...
  const generarContrato = useCallback(
    async (kilometros: string, horaRealEntrega: string) => {
      ...
      try {
        ...
        alert(`Ha ocurrido un error al generar el contrato: ${mensajeError}\n\nPor favor, inténtelo de nuevo.`)
      } finally {
    ...
    try {
      ...
      root.render(
        <ReservationA4Template
          ...
          reservaValidada={reserva.validado}
        />,
      )
  ...
  // Función para copiar el texto de confirmación formateado
  const handleCopiarTextoConfirmacion = useCallback((reserva: ReservaCompleta) => {
    try {
      const plantilla = obtenerPlantilla("confirmacion-reserva")
      if (!plantilla) {
        alert("No se encontró la plantilla de confirmación.")
        return
      }
      ...
      texto = texto.replace(/{{numeroReserva}}/g, reserva.numeroReserva)
      texto = texto.replace(/{{cliente\.nombre}}/g, reserva.cliente.nombre)
      texto = texto.replace(/{{fechaEntrega}}/g, formatearFechaSegura(reserva.detalles.fechaEntrega, "d 'de' MMMM"))
      texto = texto.replace(/{{horaEntrega}}/g, reserva.detalles.horaEntrega || "09:00")
      texto = texto.replace(/{{importeRestante}}/g, reserva.importeRestante.toString())
      texto = texto.replace(/{{fechaDevolucion}}/g, formatearFechaSegura(reserva.detalles.fechaDevolucion, "d 'de' MMMM"))
      texto = texto.replace(/{{horaDevolucion}}/g, reserva.detalles.horaDevolucion || "09:00")
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        try {
          ...
          alert("No se pudo copiar el texto automáticamente. Por favor, cópialo manualmente.")
        }
        document.body.removeChild(textArea)
      }
    } catch (error) {
      console.error("Error en handleCopiarTextoConfirmacion:", error)
      alert("Error al procesar el texto de confirmación.")
    }
  }, [])
  ...
  return (
    <main className="flex min-h-screen flex-col p-4 sm:p-8 bg-[#EBEBEB] font-body selection:bg-[#baeed9] selection:text-[#003829]">
      <div className="w-full max-w-[95%] lg:max-w-7xl mx-auto flex flex-col gap-6">
        {/* Superior Sticky */}
        <div className="sticky top-0 z-50 -mx-4 px-4 py-4 mb-2 bg-[#EBEBEB]/80 backdrop-blur-md">
                    ...
                    <div className="overflow-hidden xl:overflow-visible">
                      <div className="w-full h-full flex items-center justify-end flex-wrap gap-2 pt-3 pb-1 xl:pt-0 xl:pb-0 border-t xl:border-t-0 border-[#D1D5D2]/30 mt-1 xl:mt-0">
                        {!reserva.anulada ? (
                          <>
                            <button 
                              onClick={() => handleCopiarTextoConfirmacion(reserva)} 
                              className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-[#E5F3EA] text-[#003829] border border-[#baeed9] hover:bg-[#baeed9] transition-all font-bold text-[13px] shadow-sm whitespace-nowrap"
                              title="Una vez realizada la confirmación de la reserva puedes enviarle este texto de confirmación al cliente además del pdf de la reserva validada."
                            >
              ...
              <div className="bg-white rounded-[1.5rem] p-6 shadow-sm border border-[#EBEBEB]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex justify-between items-center"><span className="text-[#707974] text-[13px]">Depósito ({
                        reservaParaPrevisualizar.detalles.porcentaje !== undefined
                          ? reservaParaPrevisualizar.detalles.porcentaje
                          : (reservaParaPrevisualizar.importeTotal > 0
                              ? Math.round((reservaParaPrevisualizar.importeSenal / reservaParaPrevisualizar.importeTotal) * 100)
                              : 0)
                      }%)</span><span className="font-bold text-[#003829] text-[14px]">{reservaParaPrevisualizar.importeSenal}€</span></div>
            ...
            <button
              onClick={() => {
                if (reservaParaPrevisualizar) {
                  generarPDFConfirmado(reservaParaPrevisualizar)
                }
              }}
              className="px-8 py-3.5 rounded-full font-headline font-bold text-white bg-[#c62828] hover:bg-[#b71c1c] transition-all w-full sm:w-auto text-sm uppercase tracking-wider shadow-md hover:shadow-lg"
            >
              Generar PDF
            </button>
            <button
