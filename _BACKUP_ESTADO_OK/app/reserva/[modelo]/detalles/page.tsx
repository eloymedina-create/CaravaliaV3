...
import { guardarReserva, obtenerReservaPorNumeroYModelo, generarId } from "@/lib/reservas-store"
...
export default function DetallesReservaPage({ params }: { params: Promise<{ modelo: string }> }) {
  ...
  useEffect(() => {
    if (fechaEntrega && fechaDevolucion) {
      ...
      const totalCalculado = pDiario * dias + sup
      setImporteTotalCalc(totalCalculado)
      
      // Si no hay override manual, el total manual sigue al calculado
      if (!isManualOverride) {
        setImporteTotalManual(totalCalculado.toString())
      }

      // Lógica de Señal Inteligente con Vale
      const senalTeorica = Math.round((totalCalculado * 0.3) / 10) * 10
      if (valeSeleccionado) {
        // Si el vale cubre la señal, señal es 0
        const nuevaSenal = Math.max(0, senalTeorica - valeSeleccionado.importe)
        setImporteSenal(nuevaSenal)
      } else if (!esEdicion) {
        setImporteSenal(senalTeorica)
      }
    }
  }, [fechaEntrega, fechaDevolucion, precioDiario, suplemento, isManualOverride, valeSeleccionado, esEdicion])
  ...
  useEffect(() => {
    ...
    if (esEdicion) {
      ...
      if (reserva) {
        ...
        setIsManualOverride(true) // En edición, respetamos lo que venga
  ...
  const handleFechaEntregaChange = (date: Date | undefined) => setFechaEntrega(date || new Date())
  const handleFechaDevolucionChange = (date: Date | undefined) => setFechaDevolucion(date || new Date())
        ...
        <div className="bg-white rounded-t-[2.5rem] flex flex-col pt-10 pb-8 px-8 relative overflow-hidden">
          ...
          <div className="flex flex-col gap-6">
            ...
            <div className="mt-4 bg-[#E5F3EA] rounded-[1.5rem] p-5 flex flex-col gap-4 border border-[#baeed9] shadow-sm">
                ...
                <div className="pt-3 border-t border-dashed border-[#1b4d3e]/20 flex flex-col gap-2">
                   ...
                   <div className="flex justify-between items-center">
                     <span className="text-[11px] font-extrabold text-[#003829] uppercase tracking-wider">
                        {valeSeleccionado ? 'Señal restante a pagar:' : 'Señal ya entregada:'}
                     </span>
                     <span className="text-sm font-black text-[#003829] bg-white/50 px-2 py-0.5 rounded">{importeSenal.toFixed(2)}€</span>
                   </div>
                   <div className="flex justify-between items-center pt-1">
                     ...
                     <span className="text-2xl font-black text-[#c62828] drop-shadow-sm">
                       {(Number.parseFloat(importeTotalManual || "0") - (valeSeleccionado ? valeSeleccionado.importe : 0) - (valeSeleccionado ? importeSenal : importeSenal)).toFixed(2)}€
                     </span>
                   </div>
                </div>
             </div>
             ...
             {/* Botón Aplicar Vale */}
             {!valeSeleccionado && valesDisponibles.length > 0 && (
               <button 
                 onClick={() => setShowValeDialog(true)}
                 className="flex items-center justify-center gap-2 py-3 bg-[#FFF4D6] text-[#784112] rounded-[1.5rem] text-[11px] font-black uppercase tracking-widest border border-[#FFECC5] hover:bg-[#FFECC5] transition-all shadow-sm active:scale-95 mb-4 mx-8"
               >
                 <span className="material-symbols-outlined text-[18px]">card_giftcard</span>
                 ¿Tienes un Vale Regalo?
               </button>
             )}
