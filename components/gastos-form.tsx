...
export function GastosForm({ autocaravanas = [], gastoEditar, onSuccess }: GastosFormProps) {
  ...
  useEffect(() => {
    if (gastoEditar) {
      ...
      // REGLA: Seleccionar por defecto la primera autocaravana ACTIVA
      // Intentamos obtener las autocaravanas del store para saber cuáles están activas
      let modeloInicial = ""
      
      try {
        const autocaravanasData = localStorage.getItem("caravalia-autocaravanas")
        if (autocaravanasData) {
          const lista = JSON.parse(autocaravanasData)
          const primeraActiva = lista.find((a: any) => a.activa === true)
          if (primeraActiva) {
            modeloInicial = primeraActiva.modelo
          }
        }
      } catch (e) {
        console.error("Error al buscar activa para default:", e)
      }

      // Si no encontramos ninguna activa, usamos el fallback de la lista local
      if (!modeloInicial && autocaravanasLocales.length > 0) {
        modeloInicial = autocaravanasLocales[0].modelo
      }
