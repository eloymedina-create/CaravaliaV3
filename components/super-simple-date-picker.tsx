...
export function SuperSimpleDatePicker({
  ...
  // Actualizar el mes actual cuando cambia el valor o el mes por defecto
  useEffect(() => {
    if (value) {
      setCurrentMonth(value)
    } else if (defaultMonth && !open) {
      setCurrentMonth(defaultMonth)
    }
  }, [value, defaultMonth, open])
        ...
        <div className={cn(
          ...
          <div className="grid grid-cols-7 gap-1">
            {allDays.map((day, index) => {
              ...
              const isDisabled = minDate ? startOfDay(day) < startOfDay(minDate) : false
