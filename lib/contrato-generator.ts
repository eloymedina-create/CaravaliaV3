...
import { obtenerConfiguracion } from "./config-store"
...
export async function generarContratoWord(
  ...
  try {
    ...
    // Obtener la plantilla del contrato y del pie de página
    ...
    const clienteCompleto = {
      ...reserva.cliente,
      nombre: reserva.cliente.nombre || "CLIENTE DESCONOCIDO",
      dni: reserva.cliente.dni || "SIN DNI",
    ...
    const datosPlantilla = {
      ...
      porcentajeSenal: config.porcentajeSenal
    }
    ...
    // Crear el encabezado con información de la empresa (usando la plantilla pie-pagina si existe)
    const textoHeader = plantillaPie 
      ? plantillaPie.contenido 
      : "Caravalia es una firma de MyMarket Cor SLU / CIF B56133655 / Ingeniero Barbudo, nave 18 / 14013 Córdoba / Tfno: 957614918"
    ...
    const nombreCliente = (reserva.cliente.nombre || "Cliente").replace(/\s+/g, "_")
    const numeroReserva = reserva.numeroReserva || "S-N"
    const nombreArchivo = `Contrato_${nombreCliente}_RESERVA_${numeroReserva}_desde el día_${fechaEntregaFormateada}_al día_${fechaDevolucionFormateada}.docx`
