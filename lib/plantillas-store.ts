...
Queda incluido en el precio el Seguro a todo riesgo con franquicia de {{fianza}}€, la cual se entrega mediante transferencia inmediata o tarjeta.

### Tercera.- Fianza
Con anterioridad a este acto el arrendatario ha debido hacer transferencia por un importe de {{fianza}}€ como fianza para garantizar el buen uso y correcta devolución del vehículo.
...
Si los desperfectos o deterioros fueran superiores a {{fianza}} €, serán por cuenta de la cía de seguros que cubre el riesgo de la autocaravana a partir de dicha cantidad (franquicia), salvo que el arrendatario hubiese actuado con mala fe y la citada cía de seguros rehúse el abono, pues en tal caso el importe de las reparaciones serán íntegras por el arrendatario.
...
El día de la entrega se hará un bloqueo en tarjeta del total de la fianza ({{fianza}}€). El resto del importe se abonará en efectivo o por transferencia previa. El límite de kilómetros diarios es de 300km/día acumulables con lo que, como ejemplo, en 5 días el límite de km sería de 1.500km. Si se superan, el costo sería de 50€ por cada 100km de más realizados. Recuerde que es necesario para la retirada del vehículo el carnet de conducir vigente de todos los ocupantes que vayan a conducir, DNI vigente de todos los ocupantes del vehículo y justificante del pago del total de la fianza.

El cliente declara que ha leído y acepta las condiciones de la presente reserva y las condiciones generales que acompañan, cuyos términos se dan por conocidos. Esta reserva será válida solo cuando se haya realizado la entrega mediante cualquiera de los medios disponibles y quedará confirmada en el momento del pago de la señal del {{porcentajeSenal}}% del total del importe.
    `,
  },
  {
    id: "presupuesto-previo",
    nombre: "Presupuesto / Información Previa",
    tipo: "otros",
    ultimaModificacion: new Date().toISOString(),
    contenido: `Buenos días. Para las fechas que me comenta ({{fechas_o_dias}}) el precio diario sería de {{precioDiario}}€ diarios, con lo que el total del alquiler en esos días comentados sería de {{importeTotal}}€. 

Para poder hacer la reserva de la autocaravana y asegurarse la misma, necesitaría que me enviara una fotografía de ambas caras del DNI mediante correo (e.medina@mymarket.es) o mediante whatsapp (635 333 086 (Eloy)). 

Le haríamos un documento de reserva previa con un bloqueo provisional de 12 horas. Durante este tiempo podrá hacer un ingreso del {{porcentajeSenal}}% del total del alquiler por bizum, transferencia o mediante contado, el cual sería de un total de {{importeSenal}}€. 

Una vez recibido dicho importe recibirá un nuevo documento de que la reserva está confirmada.`,
  },
  {
    id: "pie-pagina",
...
Caravalia es una firma de MyMarket Cor SLU. / CIF: B56133655 / Ingeniero Barbudo, 18 / 14013 / Córdoba / Tfno: 957614918
  ...
  },
  {
    ...
    nombre: "Confirmación WhatsApp/Email",
    tipo: "otros",
    ultimaModificacion: new Date().toISOString(),
    contenido: `*Buenos días {{cliente.nombre}}.* 

Tu reserva *#{{numeroReserva}}* está confirmada. Te adjunto el comprobante. 

Te confirmo que el próximo día *{{fechaEntrega}}* nos vemos a partir de las *{{horaEntrega}}*. Ese día se te hará la explicación del funcionamiento de la autocaravana y el contrato. 

El resto del pago (*{{importeRestante}}€*), pueden ser o bien de contado o bien en transferencia, como tú quieras. 

Ese mismo día se te hará un bloqueo en tarjeta (el tarjetero es propiedad de la aseguradora, por eso solo podemos bloquear la fianza con él) de *{{fianza}}€* los cuales se desbloquearán una vez entregada la autocaravana tras comprobar que todo es correcto. 

La fecha de entrega es el *{{fechaDevolucion}}* a las *{{horaDevolucion}}h*. Cualquier duda que tengas solo tienes que llamarme. Saludos`,
  },
]
...
export function obtenerPlantillas(): PlantillaDocumento[] {
  try {
    ...
    let plantillas: PlantillaDocumento[] = []
    ...
    if (!plantillasGuardadas) {
      ...
      // ASEGURAR QUE TODAS LAS PLANTILLAS PREDETERMINADAS ESTÉN PRESENTES
      // Si el usuario ya tenía plantillas guardadas, las nuevas (como el contrato o confirmación) podrían faltar.
      let huboCambios = false
      plantillasDefault.forEach((pDefault) => {
        const existe = plantillas.some((p) => p.id === pDefault.id)
        if (!existe) {
          plantillas.push(pDefault)
          huboCambios = true
        }
      })
