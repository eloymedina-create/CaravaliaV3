...
export async function obtenerBloqueosFirebase(): Promise<any[]> {
  try {
    const db = requireDb()
    const snapshot = await getDocs(collection(db, COLECCIONES.bloqueos))
    return snapshot.docs.map((d) => d.data())
  } catch (error) {
    console.error("Firebase: Error al obtener bloqueos:", error)
    return []
  }
}

export async function guardarBloqueoFirebase(bloqueo: any): Promise<boolean> {
  try {
    const db = requireDb()
    const docRef = doc(db, COLECCIONES.bloqueos, bloqueo.id)
    const datosLimpios = limpiarObjetoParaFirebase({
      ...bloqueo,
      updatedAt: new Date().toISOString(),
    })
    ...
    console.error("Firebase: Error al guardar bloqueo:", error)
    return false
  }
}

export async function eliminarBloqueoFirebase(id: string): Promise<boolean> {
  try {
    const db = requireDb()
    await deleteDoc(doc(db, COLECCIONES.bloqueos, id))
    return true
  } catch (error) {
    console.error("Firebase: Error al eliminar bloqueo:", error)
    return false
  }
}

export function escucharBloqueosFirebase(callback: (bloqueos: any[]) => void): () => void {
  return escucharColeccionFirebase<any>(COLECCIONES.bloqueos, callback)
}
...
export async function obtenerEventosFirebase(): Promise<any[]> {
  try {
    const db = requireDb()
    const snapshot = await getDocs(collection(db, COLECCIONES.eventos))
    return snapshot.docs.map((d) => d.data())
  } catch (error) {
    console.error("Firebase: Error al obtener eventos:", error)
    return []
  }
}

export async function guardarEventoFirebase(evento: any): Promise<boolean> {
  try {
    const db = requireDb()
    const docRef = doc(db, COLECCIONES.eventos, evento.id)
    const datosLimpios = limpiarObjetoParaFirebase({
      ...evento,
      updatedAt: new Date().toISOString(),
    })
    await setDoc(docRef, datosLimpios)
    console.log("Firebase: Evento guardado:", evento.id)
    return true
  } catch (error) {
    console.error("Firebase: Error al guardar evento:", error)
    return false
  }
}

export async function eliminarEventoFirebase(id: string): Promise<boolean> {
  try {
    const db = requireDb()
    await deleteDoc(doc(db, COLECCIONES.eventos, id))
    console.log("Firebase: Evento eliminado:", id)
    return true
  } catch (error) {
    console.error("Firebase: Error al eliminar evento:", error)
    return false
  }
}

export function escucharEventosFirebase(callback: (eventos: any[]) => void): () => void {
  return escucharColeccionFirebase<any>(COLECCIONES.eventos, callback)
}
...
export async function guardarConfigFirebase(config: any): Promise<boolean> {
  try {
    const db = requireDb()
    const docRef = doc(db, COLECCIONES.config, "global")
    const datosLimpios = limpiarObjetoParaFirebase({
      ...config,
      updatedAt: new Date().toISOString(),
    })
...
export async function obtenerConfigFirebase(): Promise<any | null> {
  try {
    const db = requireDb()
    const docRef = doc(db, COLECCIONES.config, "global")
...
export function escucharConfigFirebase(callback: (config: any) => void): () => void {
  try {
    const db = requireDb()
    const docRef = doc(db, COLECCIONES.config, "global")
...
export async function migrarDatosLocalAFirebase(): Promise<{
  exito: boolean
  mensaje: string
  detalles: { reservas: number; autocaravanas: number; gastos: number; precios: number; eventos: number }
}> {
  const detalles = { reservas: 0, autocaravanas: 0, gastos: 0, precios: 0, eventos: 0 }
  ...
  try {
    ...
    if (rawVales) {
      const vales: any[] = JSON.parse(rawVales)
    ...
    // ── Bloqueos ──
    const rawBloqueos = typeof window !== "undefined" ? localStorage.getItem("caravalia-bloqueos") : null
    if (rawBloqueos) {
      const bloqueos: any[] = JSON.parse(rawBloqueos)
      for (const b of bloqueos) {
        await guardarBloqueoFirebase(b)
      }
    }

    // ── Eventos ──
    const rawEventos = typeof window !== "undefined" ? localStorage.getItem("caravalia_eventos_especiales") : null
    if (rawEventos) {
      const eventos: any[] = JSON.parse(rawEventos)
      for (const e of eventos) {
        await guardarEventoFirebase(e)
        detalles.eventos++
      }
    }

    const total = detalles.reservas + detalles.autocaravanas + detalles.gastos + detalles.precios + detalles.eventos
